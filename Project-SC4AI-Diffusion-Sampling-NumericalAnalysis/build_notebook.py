...
> **摘要.** 本文把扩散模型（score-based / SDE 生成模型）的**反向采样**重新表述为"**数值求解一个反向时间随机微分方程（SDE）/概率流常微分方程（ODE）**"的问题，并用**数值分析**的工具系统地理解与改进它。在数据为各向同性高斯的可解析设定下，反向过程是**线性 SDE**，其矩满足**闭式确定性递推**，从而每一个理论命题都有**解析真值**作硬基准、结论可证伪。我们：(i) 用**后向误差分析 / 修正方程**解析地推出 Euler–Maruyama 采样在生成分布方差上引入的领头阶 $O(h)$ 偏差及其**闭式系数**，并以 Richardson 外推独立交叉验证（二者数值完全吻合，相对误差 <0.1%）；(ii) 修正并澄清了 EM 在**加性噪声**下的**强收敛阶为 1**（而非教科书中乘性噪声的 1/2）；(iii) 用**刚性 / 绝对稳定域**第一性地解释采样步长上界 $h\le 2/a(t)$；(iv) 推导半隐式 / 指数积分器与概率流 ODE 的**闭式解** $x\propto\sqrt{v}$，并诚实评测少步采样；(v) 给出总采样误差的**三分解**（分数 / 离散 / 先验）与 **Fokker–Planck 残差**诊断；(vi) 迁移到训练所得网络 score，定量定位**离散误差与网络逼近误差的交叉点**。所有实验在 2-D 以下、单机分钟级完成（代码 device-agnostic，支持 Apple Silicon 的 torch MPS 与 CPU；网络实验默认用 CPU 以逐位可复现），每个解析命题都在对应实验 cell 内以定量断言对照解析真值。
""")
